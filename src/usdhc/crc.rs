...
///
pub fn crc7(data: &[u8], n: usize) -> u8 {
    let mut crc = 0u8;
    for i in 0..n {
        let mut d = data[i];
        for _ in 0..8 {
...
/// Uses CRC_TAB to create the checksum
/// The CRC polynomials for the data are as follows
///
/// Generator polynomial: G(x) = x^16 + x^12 + x^5 + 1
/// M(x) = (first bit) * x^n + (second bit) * x^(n-1) +...+ (last bit) * x^0
/// CRC[15:0] = Remainder [(M(x) * x^16) / G(x)]
#[cfg(not(feature = "save_memory"))]
pub fn crc_ccitt(data: &[u8], n: usize) -> u16 {
    let mut crc = 0u16;
    for i in 0..n {
        let idx = ((crc >> 8 ^ data[i] as u16) & 0xFF) as usize;
